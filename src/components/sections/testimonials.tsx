  ...
  return (
    <section className="py-24 bg-background grain">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl md:text-5xl font-display text-foreground text-center mb-16">Stories from the field.</h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {TESTIMONIALS.map((t, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-card p-8 rounded-3xl border border-border flex flex-col gap-6 shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <div className="space-y-4 flex-1">
                <p className="text-lg font-body leading-relaxed text-foreground/90 italic">"{t.quote}"</p>
                <p className="text-sm font-body text-krishi-gold">{t.hindi}</p>
              </div>

              <div className="flex items-center gap-4 pt-6 border-t border-border">
                <div className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center font-headline font-bold text-krishi-gold">
                   {t.initials}
                </div>
                <div>
                   <h4 className="font-headline font-bold text-sm text-foreground">{t.name}</h4>
                   <p className="text-xs text-foreground/40">{t.location}</p>
                </div>
              </div>
            </motion.div>
          ))}
